...
class SendvidParser: VideoProviderParser {
    var aliases: [String] { [ "SendVid", "Sendvid" ] }
    
    private static let playerSourceRegex = try! NSRegularExpression(
        pattern: "video_source\\s=\\s\"([^\"]+)",
        options: []
    )
    ...
    func parse(episode: Episode, with session: Session, forPurpose purpose: Purpose, onCompletion handler: @escaping NineAnimatorCallback<PlaybackMedia>) -> NineAnimatorAsyncTask {
        session.request(episode.target).responseString {
            ...
            do {
                ...
                let sourceUrl = try (SendvidParser
                    .playerSourceRegex
                ...
                Log.info("(Sendvid Parser) found asset at %@", resourceUrl.absoluteString)
