...
extension NASourceAnimeWorld {
    func episode(from link: EpisodeLink, with anime: Anime) -> NineAnimatorPromise<Episode> {
        NineAnimatorPromise.firstly {
            ...
            let videoElement = try bowl.select("div#download>div.widget a")
            let videoSource: URL
            if videoElement.isEmpty() {
                ...
                Log.info("[NASourceAnimeWorld] Resource found from packed scripts.")
            } else {
                let videourl_nc = try videoElement.attr("href")
                var videourl = videourl_nc.replacingOccurrences(of: "download-file.php?id=", with: "")
                ...
                Log.info("[NASourceAnimeWorld] Resource found from page source.")
            }
            return Episode(
                link,
                target: videoSource,
